...
class Generator(
    baseList: ArrayList<CourseBaseBean>,
    detailList: ArrayList<CourseDetailBean>,
    ...
    //默认配置
    private val header =
        "{\"courseLen\":50,\"id\":1,\"name\":\"${timeTable?.name ?: "默认"}\",\"sameBreakLen\":false,\"sameLen\":true,\"theBreakLen\":10}"

    //课程时间配置
    private val defaultTimePreference =
    ...
    private val colorScheme =
        "{\"background\":\"\",\"courseTextColor\":-1,\"id\":1,\"itemAlpha\":60,\"itemHeight\":64,\"itemTextSize\":12,\"maxWeek\":${maxWeek ?: 20},\"nodes\":${nodes ?: 20},\"showOtherWeekCourse\":true,\"showSat\":true,\"showSun\":true,\"showTime\":false,\"startDate\":\"${startDate ?: "2021-9-6"}\",\"strokeColor\":-2130706433,\"sundayFirst\":false,\"tableName\":\"${tableName ?: "未命名"}\",\"textColor\":-16777216,\"timeTable\":1,\"type\":0,\"widgetCourseTextColor\":-1,\"widgetItemAlpha\":60,\"widgetItemHeight\":64,\"widgetItemTextSize\":12,\"widgetStrokeColor\":-2130706433,\"widgetTextColor\":-16777216}"
    ...
    private fun timePreference(): String {
        return timeTable?.timeList?.joinToString(",", "[", "]") {
            "{\"endTime\":\"${it.endTime}\",\"node\":${it.node},\"startTime\":\"${it.startTime}\",\"timeTable\":1}"
        }
